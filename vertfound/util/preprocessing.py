...
from vertfound.modeling.segment_anything.utils.transforms import ResizeLongestSide
...
def prepare_prompt_train(batched_inputs, target_size=(224,224)):
    ...
    for x in batched_inputs:
        ...
        curr_size = (image.shape[0], image.shape[1])
        resized_image = resize_and_normalize(raw_image.cuda() / 255, target_size=target_size) # [3, 224, 224]
        ...
        mask_tokens = torch.tensor(x["extra_info"]['mask_tokens']).clone().detach().cuda() # [?, 1, 256]
        labels = torch.tensor(x["extra_info"]['classes']).cuda()

        # Decrement each label by 1 unless it's zero,比如原来是[1,2,3,4,5,6,7] -> [0,1,2,3,4,5,6]
...
def prepare_logits(logits):
    ...
    dp = torch.zeros(logits.shape, device=device)
    for i in range(dp.size(0)):
        dp[i, 0, :] = logits[i, 0, :]
        for j in range(1, dp.size(1)):
            for k in range(1, dp.size(2)):
                dp[i, j, k] = torch.max(dp[i, j - 1, k - 1] + logits[i, j, :k + 1] * torch.sigmoid(torch.arange(0, k + 1, 1, device=device)))
    return dp