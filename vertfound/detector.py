...
from .modeling.vertfound import build_vertfound_model
...
class VertFound(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.model, _ = build_vertfound_model(clip_type = self.clip_type, is_training=cfg.MODEL.TRAINING, image_size=self.clip_input_size)
    ...
    def foward_inference(self, batched_inputs, do_postprocess=True):
        with amp.autocast(enabled=True):
            with torch.no_grad():
                logits_per_image = self.model.forward_eval(batched_inputs, multimask_output=False)
        ...
        box_pred = boxes
        # print(box_cls.shape, box_pred.shape)
        if self.inference_box_type == 'GT':
            results = self.inference_gt_box_new(box_cls, box_pred, image_sizes)
        else:
            results = self.inference_pred_box_new(box_cls, box_pred, scores, image_sizes)
        if do_postprocess:
            processed_results = []
    ...
    def foward_train(self, batched_inputs):
        ...
        loss = {'ce_loss': outputs[0], 'cn_loss': outputs[1], 'sk_loss': outputs[2]}
        return loss
    ...
    def inference_gt_box(self, box_cls, box_pred, pred_mask, image_sizes=None):
        ...
        for logits, boxes, masks, img_size in zip(box_cls, box_pred, pred_mask, image_sizes):
            ...
            top_num = min(700, len(probs_flattened))
    ...
    def inference_gt_box_new(self, box_cls, box_pred, image_sizes=None):

        device = box_cls.device  # assuming all tensors are on the same device
        results = []

        for logits, boxes, img_size in zip(box_cls, box_pred, image_sizes):
            # Calculate probabilities and flatten them
            probs = F.softmax(logits, dim=-1)
            box_num = probs.shape[0]
            target = 0
            x=0
            for i in range(probs.shape[1]-box_num):
                probs_sum=probs[:,i:box_num+i].sum()
                if probs_sum > x:
                    target = i
                    x=probs_sum
            index_min = max(0,target-2)
            index_max = min(probs.shape[1],target+box_num+2)
            # probs=probs[:,index_min:index_max]
            probs_flattened = probs.reshape(-1)
            
            
            # Determine number of top predictions to consider
            top_num = min(250, len(probs_flattened))
            ...
            # Decode the top-k indices to get corresponding labels and boxes
            topk_labels = topk_indices % probs.shape[1]
            # topk_labels = topk_labels + index_min
            topk_boxes_indices = topk_indices // probs.shape[1]
    ...
    def inference_pred_box_new(self, box_cls, box_pred, box_score, image_sizes=None):

        results = []

        for i, (logits, box_pred_i, box_score_i, img_size) in enumerate(zip(box_cls, box_pred, box_score, image_sizes)):
